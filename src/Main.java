...
public class Main extends JFrame {
	...
	public static void getAllGroups() {
		...
		for (Group group : storage.AllGroups) {
		...
		tableModel.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				...
				for (int i = 0; i < storage.AllGroups.size(); i++) {
					if (data.equals(storage.AllGroups.get(i).name)) {
						sameName = true;
						break;
					}
				}
				String nameOfGroup = storage.AllGroups.get(row).name;
				if (column == 0) {
					if (!sameName) {
						storage.AllGroups.get(row).name = data;
					} else {
						JOptionPane.showMessageDialog(main, "Таке ім'я групи товарів вже існує!");
						model.setValueAt(nameOfGroup, row, column);
					}
				} else storage.AllGroups.get(row).description = data;
				model.addTableModelListener(this);
			}
		});
		...
		add.addActionListener(e -> {
			...
			if (result == JOptionPane.OK_OPTION) {
				...
				boolean sameName = false;
				for (int i = 0; i < storage.AllGroups.size(); i++) {
					if (group.name.equals(storage.AllGroups.get(i).name)) {
						sameName = true;
						break;
					}
				}
		...
		search.addActionListener(e -> {
			...
			if (result == JOptionPane.OK_OPTION) {
				flag = false;
				for (int i = 0; i < storage.AllGroups.size(); i++) {
					for (int j = 0; j < storage.AllGroups.get(i).products.size(); j++) {
						// if (nameField.getText().contains(s.AllGroups.get(i).products.get(j).getName())){
						boolean flag2 = true;
						for (int u = 0; u < nameField.getText().length() && u < storage.AllGroups.get(i).products.get(j).getName().length(); u++) {
							if (!(nameField.getText().charAt(u) == (storage.AllGroups.get(i).products.get(j).getName().charAt(u))))
								flag2 = false;
						}
						if (flag2) {
							//System.out.println(s.AllGroups.get(i).products.get(j).toString());
							parameterGroup = storage.AllGroups.get(i);
							parameterProduct = storage.AllGroups.get(i).products.get(j);
							helper = storage.AllGroups.get(i).products.get(j).getName();
							i1 = i;
							j1 = j;
							if (!Main.flag) {
		...
		statistics.addActionListener(e -> {
			...
			for (int i = 0; i < storage.AllGroups.size(); i++) {
				for (int j = 0; j < storage.AllGroups.get(i).products.size(); j++) {
					parameterStatisticGroup = storage.AllGroups.get(i);
					parameterStatisticProduct = storage.AllGroups.get(i).products.get(j);
					i2 = i;
					j2 = j;
		...
		changeGroupsProducts.addActionListener(e -> {
			int idx = table.getSelectedRow();
			if (idx < 0) return;
			getProductTable(storage.AllGroups.get(idx));
		});
	...
	private static void getProductTable(Group group) {
			...
			public void tableChanged(TableModelEvent e) {
				...
				switch (column) {
					...
					case 0 -> {
						...
						for (int i = 0; i < storage.AllGroups.size(); i++) {
							for (int j = 0; j < storage.AllGroups.get(i).products.size(); j++) {
								if (data.equals(storage.AllGroups.get(i).products.get(j).getName())) {
									sameName = true;
									break;
								}
							}
		...
		add.addActionListener(e -> {
			...
			if (result == JOptionPane.OK_OPTION) {
				...
				for (int i = 0; i < storage.AllGroups.size(); i++) {
					for (int j = 0; j < storage.AllGroups.get(i).products.size(); j++) {
						if (product.name.equals(storage.AllGroups.get(i).products.get(j).getName())) {
							sameName = true;
							break;
						}
					}
	...
	public static void main(String[] args) {
		main = new Main();
		storage = new Storage();
		int statusCode = storage.loadGroups(main);
		if (statusCode != -1) {
			getAllGroups();
