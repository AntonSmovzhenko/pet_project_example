...
public class makeStatistics extends JFrame {
	...
	public makeStatistics() {
		...
		if (!Main.groupStatistic) {
			for (int u = 0; u < Main.storage.AllGroups.size(); u++) {
				for (int w = 0; w < Main.storage.AllGroups.get(u).products.size(); w++) {
					counter += Main.storage.AllGroups.get(u).products.get(w).amount * Main.storage.AllGroups.get(u).products.get(w).price;
				}
			}
		} else {
