	...
	public toSearch() {
		...
		tableModel.addTableModelListener(new TableModelListener() {
			...
			public void tableChanged(TableModelEvent e) {
				...
				for (int i = 0; i < Main.storage.AllGroups.size(); i++) {
					if (tableModel.getValueAt(row, 1).equals(Main.storage.AllGroups.get(i).name)) {
						thisGroupCounter = Main.storage.AllGroups.get(i);
						break;
					}
				}
